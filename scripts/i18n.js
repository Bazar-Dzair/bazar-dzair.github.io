...
(function (global) {
  ...
  var UI_DICT = {
    ...
    badge_featured:            { ar: "⭐ مميز",                                   fr: "⭐ Vedette" },
    badge_offer:               { ar: "🔥 عرض",                                    fr: "🔥 Promo" },
    badge_best:                { ar: "🏆 الأكثر مبيعًا",                          fr: "🏆 Meilleure vente" },
  ...
  // شارة المنتج (productBadge) تُخزَّن في Firestore كـ"كود" ثابت (new/featured/offer/best)
  // من قائمة لوحة التحكم، وليس كنص جاهز للعرض. هذه الدالة تترجم الكود إلى النص المناسب
  // للغة الحالية. بعض المنتجات القديمة قد تحتوي على حقل "badge" قديم بنص عربي حر مباشر
  // (من قبل اعتماد نظام الأكواد) — في هذه الحالة (الكود غير معروف) نعرض النص كما هو.
  var BADGE_CODE_KEYS = { new: "badge_new", featured: "badge_featured", offer: "badge_offer", best: "badge_best" };
  function badgeLabel(code) {
    if (!code) return "";
    var key = BADGE_CODE_KEYS[code];
    if (key) return t(key);
    return String(code);
  }
